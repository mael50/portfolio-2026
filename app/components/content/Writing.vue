...
const { data: articles } = await useAsyncData('articles-' + locale.value, async () => {
  ...
  const result = await queryCollection(collection).all() as Collections['articles_en'][] | Collections['articles_fr'][]
    ...
    <TransitionGroup v-if="filteredArticles.length" name="list" tag="ul" class="grid grid-cols-1 gap-4 sm:grid-cols-2">
      <li v-for="article of filteredArticles" :key="article.path">
        <ArticleCard :title="article.title" :date="article.date" :image="article.image"
          :path="article.path || `/${locale}/articles/${article.stem?.split('/').pop()}`" />
      </li>
    </TransitionGroup>
