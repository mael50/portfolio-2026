...
<template>
  <NuxtLink :aria-label="`${project.name} project link`" :to="localePath('/projects/' + project.stem.split('/').pop())"
      ...
      <div class="mt-auto flex flex-wrap items-center gap-2 pt-2">
        <span class="ml-auto flex items-center gap-2 text-[10px] uppercase tracking-[0.2em] text-white/60">
          ...
          <UIcon name="heroicons:arrow-right"
            class="size-3 text-white/60 transition-transform group-hover:-rotate-45" />
        </span>
      </div>
    </div>
  </NuxtLink>
