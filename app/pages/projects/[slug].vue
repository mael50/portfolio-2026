...
<template>
  <div v-if="project" class="relative py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
    <div
    ...
    <div class="relative z-10 flex flex-col gap-8">
      <button class="flex items-center gap-2 text-white/60 hover:text-white transition-colors w-fit cursor-pointer"
        @click="goBack">
      ...
      <div class="flex flex-col gap-4">
        ...
        <div class="flex items-center gap-4 text-white/60">
          ...
          <span v-if="project.link" class="w-1 h-1 bg-white/60 rounded-full" />
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div v-if="project.about_client" class="flex flex-col gap-4">
          <h2 class="text-2xl font-bold text-white">
            {{ $t('global.about_client') }}
          </h2>
          <p class="text-lg text-white/80 leading-relaxed">
        ...
        <div v-if="project.about_project" class="flex flex-col gap-4">
          <h2 class="text-2xl font-bold text-white">
            {{ $t('global.about_project') }}
          </h2>
          <p class="text-lg text-white/80 leading-relaxed">
      ...
      <div v-if="project.gallery?.length && project.gallery.length > 1" class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div v-for="(img, index) in project.gallery" :key="index"
          class="rounded-lg overflow-hidden border border-white/10 bg-white/5">
          <NuxtImg :src="img" class="w-full h-auto object-cover hover:scale-105 transition-transform duration-500" />
        </div>
      </div>
    </div>
  </div>
